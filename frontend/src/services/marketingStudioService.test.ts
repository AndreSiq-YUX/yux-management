...
import {
  ...
  buildAgentPayload,
  buildCampaignCreativeSuggestionPayload,
  buildCampaignDraftRunPayload,
  ...
  mapMarketingCampaignCreativeSuggestion,
  mapMarketingCampaignDraftRun,
  mapMarketingContent,
...
describe('marketingStudioService mapping helpers', () => {
  ...
  it('maps campaign creative suggestions and draft run payloads', () => {
    expect(mapMarketingCampaignCreativeSuggestion({
      id: 'suggestion-1',
      organization_id: 'org-1',
      client_id: 'client-1',
      contract_id: 'contract-1',
      ...
      title: 'Campanha CRM',
      campaign_name: 'CRM para PMEs',
      angle: 'Reduzir retrabalho comercial',
      target_audience: 'Donos de PMEs',
      ...
      utm_source: 'meta',
      utm_medium: 'paid',
      utm_campaign: 'crm_para_pmes',
      copy_variations: [{ headline: 'CRM sem bagunca' }],
      creative_concepts: [{ name: 'Dashboard limpo' }],
      targeting_suggestions: { interests: ['crm'] },
      quality_score: 84,
      risk_flags: [],
      ...
      copyVariations: [{ headline: 'CRM sem bagunca' }],
    })

    expect(buildCampaignCreativeSuggestionPayload({
      organizationId: 'org-1',
      clientId: 'client-1',
      contractId: 'contract-1',
      title: ' Campanha CRM ',
      ...
      dailyBudget: 80,
      copyVariations: [{ headline: 'CRM sem bagunca' }],
      creativeConcepts: [{ name: 'Dashboard limpo' }],
    })).toMatchObject({
      title: 'Campanha CRM',
      campaign_name: 'CRM para PMEs',
      provider: 'meta',
    ...
    expect(mapMarketingCampaignDraftRun({
      id: 'draft-run-1',
      organization_id: 'org-1',
      client_id: 'client-1',
      contract_id: 'contract-1',
      ...
      workflow_run_id: null,
      status: 'succeeded',
      idempotency_key: 'suggestion-1:new_campaign:latest',
      request_payload: {},
      response_payload: { campaignId: 'campaign-1' },
      protected_error: null,
      requested_by: null,
      started_at: null,
      completed_at: '2026-06-07T12:00:00.000Z',
      created_at: '2026-06-07T12:00:00.000Z',
      updated_at: '2026-06-07T12:00:00.000Z',
    })).toMatchObject({ suggestionId: 'suggestion-1', campaignId: 'campaign-1', status: 'succeeded' })

    expect(buildCampaignDraftRunPayload({
      organizationId: 'org-1',
      clientId: 'client-1',
      contractId: 'contract-1',
      suggestionId: 'suggestion-1',
      idempotencyKey: 'suggestion-1:new_campaign:latest',
    })).toMatchObject({
      suggestion_id: 'suggestion-1',
      status: 'queued',
      idempotency_key: 'suggestion-1:new_campaign:latest',
