...
class CrmConversationQueryBuilder<T = any[]> implements PromiseLike<QueryResult<T>> {
  ...
  single() {
    ...
    return this as unknown as CrmConversationQueryBuilder<any>
  }
  ...
  private async execute(): Promise<QueryResult<T>> {
    try {
      const response = await apiRequest<QueryResult<T>>('/crm/conversation-query', {
        method: 'POST',
        body: {
          table: this.table,
...
export const crmConversationDataClient = {
  from<T = any>(table: string) {
    return new CrmConversationQueryBuilder<T[]>(table)
  },
}
