...
class CampaignQueryBuilder<T = any[]> implements PromiseLike<QueryResult<T>> {
  ...
  single() {
    ...
    return this as unknown as CampaignQueryBuilder<any>
  }
  ...
  private async execute(): Promise<QueryResult<T>> {
    try {
      return await apiRequest<QueryResult<T>>('/campaigns/query', {
        method: 'POST',
        body: {
          table: this.table,
...
export const campaignDataClient = {
  from<T = any>(table: string) {
    return new CampaignQueryBuilder<T[]>(table)
  },
}
