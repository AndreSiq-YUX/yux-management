...
class CrmOpsQueryBuilder<T = any[]> implements PromiseLike<QueryResult<T>> {
  ...
  single() {
    ...
    return this as unknown as CrmOpsQueryBuilder<any>
  }
  ...
  private async execute(): Promise<QueryResult<T>> {
    try {
      return await apiRequest<QueryResult<T>>('/crm/ops-query', {
        method: 'POST',
        body: {
          table: this.table,
...
export const crmOpsDataClient = {
  from<T = any>(table: string) {
    return new CrmOpsQueryBuilder<T[]>(table)
  },
}
