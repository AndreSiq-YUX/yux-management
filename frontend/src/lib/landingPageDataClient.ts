...
class LandingPageQueryBuilder<T = any[]> implements PromiseLike<QueryResult<T>> {
  ...
  single() {
    ...
    return this as unknown as LandingPageQueryBuilder<any>
  }
  ...
  private async execute(): Promise<QueryResult<T>> {
    try {
      return await apiRequest<QueryResult<T>>('/landing-pages/query', {
        method: 'POST',
        body: {
          table: this.table,
...
export const landingPageDataClient = {
  from<T = any>(table: string) {
    return new LandingPageQueryBuilder<T[]>(table)
  },
}
