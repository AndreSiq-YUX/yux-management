...
export function AdminSystemEmailTemplatesPage() {
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      const [templateResult, sendRequestResult] = await Promise.all([
        emailTemplateService.listAdminTemplates(),
        emailTemplateService.listAdminSendRequests(),
      ])
      setTemplates(templateResult)
      setSendRequests(sendRequestResult)
    } catch (cause) {
      console.error('Error loading system email templates:', cause)
      setError('Nao foi possivel carregar os modelos de email do sistema.')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  if (loading && templates.length === 0) {
    return <p className="text-sm text-gray-600">Carregando modelos de email do sistema...</p>
  }
