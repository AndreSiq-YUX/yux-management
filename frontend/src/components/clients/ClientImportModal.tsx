...
export function ClientImportModal({ isOpen, onClose, onSuccess }: ClientImportModalProps) {
  ...
  const handleFileSelect = (selectedFile: File) => {
    ...
    if (!isCsv) {
      toast.error('Formato nao suportado. Use CSV.')
      return
    }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (let index = 1; index < lines.length; index++) {
        ...
        const payload = {
          companyName: getValue(row, ['companyName', 'company_name', 'empresa']),
          contactName: getValue(row, ['contactName', 'contact_name', 'contato']),
          email: getValue(row, ['email']),
          phone: getValue(row, ['phone', 'telefone']),
          website: getValue(row, ['website', 'site']),
          sector: getValue(row, ['sector', 'setor']) || 'Outros',
          size: (getValue(row, ['size', 'porte']) || 'small') as 'small' | 'medium' | 'large',
          leadSource: getValue(row, ['leadSource', 'lead_source', 'origem']) || 'Importacao CSV',
          notes: getValue(row, ['notes', 'observacoes']),
        }
        ...
        if (!payload.companyName || !payload.contactName || !payload.email) {
          errors.push({
            row: index + 1,
            field: 'required',
            message: 'companyName, contactName e email sao obrigatorios',
          })
          continue
        }
      ...
      if (importResult.success) {
        ...
        toast.error('Importacao concluida com erros. Verifique os detalhes.')
      }
    } catch (error) {
  ...
  const handleDownloadTemplate = () => {
    const csv = [
      'companyName,contactName,email,phone,website,sector,size,leadSource,notes',
      'Empresa Exemplo,Ana Silva,ana@empresa.com,(11) 99999-9999,https://empresa.com,Tecnologia,small,Indicacao,Observacoes',
    ].join('\n')
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-medium text-blue-900">Template de Importacao</h3>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Arquivo de Clientes</label>
            <div
            ...
            >
              ...
              <div className="space-y-2">
                <p className="text-sm text-gray-600">Arraste e solte seu CSV aqui, ou</p>
                <button
                  type="button"
                ...
                <p className="text-xs text-gray-500">Formato suportado: CSV - max: 5MB</p>
              </div>
              ...
              <div className={`p-4 rounded-lg border ${result.success ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
                ...
                <div className="mt-2 text-sm">
                  ...
                  {result.errors.length > 0 && (
                    <p className="text-red-700">{result.errors.length} erros encontrados</p>
                  )}
                </div>
              </div>
