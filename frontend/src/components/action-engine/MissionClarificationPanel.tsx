...
export function MissionClarificationPanel({ questions, context, canWrite, busy, onSubmit }: {
  ...
  const [answers, setAnswers] = useState<Record<string, unknown>>(() => Object.fromEntries(
    visibleQuestions.map(question => [question.key, question.defaultValue ?? '']),
  ))
  const complete = visibleQuestions.every(question => {
  ...
  return (
    <section className="border border-amber-200 bg-amber-50 p-5">
      ...
      <div className="mt-5 grid gap-4">{visibleQuestions.map(question => <label key={question.key} className="space-y-2"><span className="block text-sm font-semibold text-slate-900">{question.label}</span><span className="block text-xs text-slate-600">{question.whyNeeded}</span>{question.defaultSourceId ? <span className="block text-xs font-medium text-amber-800">Sugerido a partir do Contexto da Empresa. Você pode alterar.</span> : null}{question.answerType === 'boolean' ? <select className="h-10 w-full border border-amber-300 bg-white px-3 text-sm" value={String(answers[question.key] ?? '')} onChange={event => setAnswers(current => ({ ...current, [question.key]: event.target.value === 'true' }))}><option value="">Selecione</option><option value="true">Sim</option><option value="false">Não</option></select> : <input className="h-10 w-full border border-amber-300 bg-white px-3 text-sm outline-none focus:border-blue-500" type={['number','currency'].includes(question.answerType) ? 'number' : question.answerType === 'date' ? 'date' : 'text'} value={String(answers[question.key] ?? '')} onChange={event => setAnswers(current => ({ ...current, [question.key]: event.target.value }))} />}</label>)}</div>
