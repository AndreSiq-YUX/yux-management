...
export function MissionsWorkspace({ organizationId, contractId, canWrite, detailHref }: { organizationId: string; contractId?: string; canWrite: boolean; detailHref: (id: string) => string }) {
  ...
  return <><MissionDashboard missions={missions} economicsByMission={economics} detailHref={detailHref} canCreate={canWrite} onCreate={() => setWizardOpen(true)} /><MissionCreateWizard open={wizardOpen} organizationId={organizationId} contractId={contractId} onOpenChange={setWizardOpen} onCreated={mission => setMissions(current => [mission, ...current])} /></>
}
