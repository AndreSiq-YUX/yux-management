...
import { resolveCorrectionTarget } from '@/lib/workspace/correctionTargets'
...
export function MissionReadinessBlockers({ readiness }: { readiness: MissionReadiness }) {
  ...
  return <section className="border border-slate-200 bg-white"><div className="border-b border-slate-200 px-5 py-4"><h2 className="font-semibold text-slate-950">Prontidão operacional</h2><p className="mt-1 text-xs text-slate-500">Corrija bloqueios antes da próxima execução.</p></div><div className="divide-y divide-slate-200">{visible.map(check => { const target = check.correctionTarget ? resolveCorrectionTarget(check.correctionTarget) : null; const path = target?.path ? portalPath(target.path) : null; return <div key={`${check.code}:${check.capabilityKey ?? ''}`} className="flex items-start gap-3 p-4"><AlertCircle className={`mt-0.5 h-4 w-4 shrink-0 ${check.status === 'block' ? 'text-red-600' : 'text-amber-600'}`} /><div className="min-w-0 flex-1"><p className="text-sm font-medium text-slate-900">{check.message}</p>{path ? <Link to={path} className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-700">Corrigir agora <ArrowRight className="h-3 w-3" /></Link> : null}</div></div> })}</div></section>
}
