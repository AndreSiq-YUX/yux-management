import { Bot, ExternalLink, GitBranch, Mail, RefreshCw, Route, ShieldAlert } from 'lucide-react'
...
export function MissionArtifactsPanel({ artifacts, canWrite, showTechnicalProof, onRefresh, refreshing = false, destinationHref }: {
  ...
  if (campaignArtifacts.length && !standardArtifacts.length) return <CampaignMissionArtifacts artifacts={campaignArtifacts} canWrite={canWrite} showTechnicalProof={showTechnicalProof} destinationHref={destinationHref} />
  const stale = standardArtifacts.some(artifact => artifact.staleApproval)
  return (
    <div className="space-y-6">
      {campaignArtifacts.length ? <CampaignMissionArtifacts artifacts={campaignArtifacts} canWrite={canWrite} showTechnicalProof={showTechnicalProof} destinationHref={destinationHref} /> : null}
    <section className="border border-slate-200 bg-white">
      ...
      <div className="grid gap-px bg-slate-200 lg:grid-cols-2">{standardArtifacts.map(artifact => <ArtifactCard key={`${artifact.kind}:${artifact.key}`} artifact={artifact} showTechnicalProof={showTechnicalProof} destinationHref={destinationHref} />)}</div>
    </section>
    </div>
  )
}

function ArtifactCard({ artifact, showTechnicalProof, destinationHref }: { artifact: MissionArtifact; showTechnicalProof: boolean; destinationHref?: (artifact: { kind: string; entityId?: string }) => string | undefined }) {
  const Icon = artifact.kind === 'funnel' ? GitBranch : artifact.kind === 'email' ? Mail : artifact.kind === 'sequence' ? Route : Bot
  const href = artifact.entityId && destinationHref ? destinationHref(artifact) : undefined
  return (
    <article className="bg-white p-5">
      ...
      {href ? <Link className="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-blue-700 hover:text-blue-900" to={href}>Abrir no sistema <ExternalLink className="h-3.5 w-3.5" /></Link> : null}
    </article>
  )
}
