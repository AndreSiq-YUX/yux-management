    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto rounded-sm">
        ...
        {!canWrite ? <div className="border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Seu perfil pode acompanhar missões, mas não criar ou alterar uma.</div> : <>
          ...
          {step === 1 ? <div className="space-y-5 py-2"><button type="button" onClick={selectRevenueRecovery} className={`flex w-full items-start gap-3 border p-4 text-left ${quickStart === 'revenue_recovery' ? 'border-blue-500 bg-blue-50' : 'border-slate-200 hover:border-blue-300'}`}><Sparkles className="mt-0.5 h-5 w-5 text-blue-600" /><span><span className="block text-sm font-semibold text-slate-900">Atalho: Revenue Recovery</span><span className="mt-1 block text-xs leading-5 text-slate-600">Preenche um objetivo inicial para recuperar oportunidades inativas. Você pode editar tudo.</span></span></button><label className="block space-y-2"><span className="text-sm font-semibold text-slate-900">Descreva o resultado desejado</span><textarea aria-label="Descreva o resultado desejado" className="min-h-36 w-full border border-slate-300 p-3 text-sm leading-6 outline-none focus:border-blue-500" placeholder="Ex.: crie um funil comercial com quatro etapas e automatize uma sequência de quatro e-mails para novos leads" value={objective} onChange={event => { setObjective(event.target.value); setQuickStart(undefined) }} /><span className="block text-xs text-slate-500">O agente fará no máximo três perguntas de esclarecimento, agrupadas.</span></label><div className="grid gap-4 sm:grid-cols-2"><TextField label="Título opcional" value={title} onChange={setTitle} placeholder="Será sugerido a partir do objetivo" /><TextField label="Valor esperado opcional (R$)" value={expectedValueBrl} onChange={setExpectedValueBrl} type="number" placeholder="0" /></div></div> : <div className="space-y-5 py-2"><AutonomyEnvelopeForm value={envelope} onChange={setEnvelope} /><div className="border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Antes de executar</p><p className="mt-2 text-sm leading-6 text-slate-700">Conexões, permissões, consentimento, ownership, orçamento e catálogo serão verificados novamente imediatamente antes de cada ação.</p>{envelopeBlockers.length > 0 ? <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-red-700">{envelopeBlockers.map(blocker => <li key={blocker}>{blocker}</li>)}</ul> : null}</div></div>}
