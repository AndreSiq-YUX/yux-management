...
import type { ActionMission, CreateMissionIntentInput, MissionRecipe } from '@/types/actionEngine'
...
export function MissionIntake({ open, organizationId, contractId, canWrite, onOpenChange, onCreated }: MissionIntakeProps) {
  ...
  const [recipe, setRecipe] = useState<MissionRecipe>()
  ...
  function selectCampaignLaunch() {
    setRecipe(undefined)
    setQuickStart('campaign_launch')
    setTitle('Lançar campanha paga completa')
    setObjective('Criar uma campanha paga completa com público, criativos, landing page, formulário e tracking; preparar no provedor pausada para aprovação antes de ativar.')
    setExpectedValueBrl('10000')
    setEnvelope(current => ({ ...current, allowedModules: ['campaigns', 'landing_pages', 'campaign_launch_agent'], mode: 'assisted' }))
  }

  function selectRecipe(selected: MissionRecipe) {
    const defaults = selected.defaultGoal
    setRecipe(selected); setQuickStart('funnel_nurture')
    setTitle(typeof defaults.title === 'string' ? defaults.title : selected.title)
    setObjective(typeof defaults.objective === 'string' ? defaults.objective : '')
    setExpectedValueBrl(typeof defaults.expectedValueBrl === 'string' ? defaults.expectedValueBrl : '')
    setEnvelope(current => ({
      ...current,
      mode: defaults.mode === 'shadow' || defaults.mode === 'prepare' || defaults.mode === 'assisted' || defaults.mode === 'autonomous' ? defaults.mode : current.mode,
      allowedModules: Array.isArray(defaults.allowedModules) ? defaults.allowedModules.filter((item): item is string => typeof item === 'string') : current.allowedModules,
      maxTotalCostBrl: typeof defaults.maxTotalCostBrl === 'string' ? defaults.maxTotalCostBrl : current.maxTotalCostBrl,
      maxHumanHours: typeof defaults.maxHumanHours === 'string' ? defaults.maxHumanHours : current.maxHumanHours,
      maxExternalContacts: typeof defaults.maxExternalContacts === 'number' ? String(defaults.maxExternalContacts) : current.maxExternalContacts,
    }))
  }
  ...
  async function createIntent() {
    ...
    try {
      const mission = await actionEngineService.createMissionIntent({
        ...
        ...(recipe ? { recipeSelection: { key: recipe.key, version: recipe.version, contentHash: recipe.contentHash } } : {}),
      })
      onCreated(mission); onOpenChange(false)
      setStep(1); setTitle(''); setObjective(''); setExpectedValueBrl(''); setQuickStart(undefined); setRecipe(undefined); setEnvelope(defaultEnvelope())
    } catch (cause) {
  ...
  function updateObjective(value: string) {
    setObjective(value); setQuickStart(undefined); setRecipe(undefined)
    ...
    if (/(campanha|meta\s*ads?|google\s*ads?|m[ií]dia\s+paga|an[uú]ncios?)/iu.test(value)) {
      setEnvelope(current => ({ ...current, allowedModules: [...new Set([...current.allowedModules, 'campaigns', 'landing_pages', 'campaign_launch_agent'])] }))
    }
  }
