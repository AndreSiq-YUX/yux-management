...
export function MissionDetailWorkspace({ missionId, organizationId, backHref, canWrite }: { missionId: string; organizationId: string; backHref: string; canWrite: boolean }) {
  ...
  async function run(key: string, work: () => Promise<unknown>, refresh = true) { setBusy(key); setError(null); try { await work(); if (refresh) await load() } catch (cause) { setError(cause instanceof Error ? cause.message : 'A operação não pôde ser concluída.') } finally { setBusy(undefined) } }
