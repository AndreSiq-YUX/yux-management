...
Deno.test('encrypts and decrypts provider secret values without leaking plaintext', async () => {
  const key = randomKey()
...
Deno.test('validates provider secret encryption key length', () => {
...
Deno.test('builds deterministic provider secret references by connection table', () => {
...
Deno.test('stores and loads provider secrets through the admin client contract', async () => {
  const key = randomKey()
