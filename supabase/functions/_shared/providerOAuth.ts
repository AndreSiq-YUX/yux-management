...
async function listGoogleAdsCustomers(input: {
  ...
  const developerToken = requireString(input.googleAdsDeveloperToken || Deno.env.get('GOOGLE_ADS_DEVELOPER_TOKEN'), 'GOOGLE_ADS_DEVELOPER_TOKEN')
  const apiVersion = optionalString(input.googleAdsApiVersion) || Deno.env.get('GOOGLE_ADS_API_VERSION') || 'v22'
