...
| --- | --- | --- | --- | --- |
| Mission Supervisor Releases 0–2 | Implemented and locally validated | `/missions`, `/portal/missoes`, `/mission-simulation/review/:token`, `/api/action-engine/*` | Migrations `0128`–`0139`, safety/knowledge/decision foundations, `funnel_nurture@1.0.0`, CRM funnel/templates/sequences/flow capabilities, immutable output bindings, artifact review UI, adversarial corpus, E2E and release runbooks | Release 2 is the first bounded operational vertical: Funnel + Nurture. Apply migrations through `0139`, enable `action_engine` + `crm` + `automations` + `funnel_nurture_agent` only for the pilot, release publication capabilities individually and complete authenticated VPS acceptance/rollback. Campaigns, composition and autonomous canary remain later releases. |
