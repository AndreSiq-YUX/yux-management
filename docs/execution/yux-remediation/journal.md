...
- Commits: preparação e gate `6e8191b`; repetição controlada do instalador de SBOM `d066704`.
...
- Aceite persistente: execução GitHub Actions `34067099016`, commit `d066704`, conclusão `success` nos oito jobs. Backend, integração persistente, frontend, Agent Runtime, J1–J7 e as três imagens/SBOMs passaram. A execução anterior revelou uma falha transitória no instalador Syft somente na matriz frontend; build e scan haviam passado. A CI agora tenta esse instalador uma única vez adicional e continua falhando se a repetição não concluir.
