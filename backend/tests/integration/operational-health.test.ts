...
it('separa readiness de saúde operacional e não transforma custo desconhecido em zero', async () => {
  ...
  try {
    ...
    await storePlatformProviderSecret(pool,{
    ...
    const denied = await rig.request('client_admin_A','GET','/api/health/operational')
    expect(denied.statusCode).toBe(403)
    const snapshot = await rig.request('yux_admin','GET','/api/health/operational')
    ...
    expect(JSON.stringify(snapshot.body)).not.toContain('integration-jina-api-key')
