...
} from '../../src/lib/edge-compat/adsProvider.js'
...
it('builds provider mutation idempotency keys', () => {
...
it('sanitizes protected provider errors', () => {
  assertEquals(sanitizeProviderError(new Error('token abc123 failed')).includes('abc123'), false)
})

it('normalizes provider mutation responses without raw credentials', () => {
...
it('rejects provider connections that need reauth', () => {
...
it('builds Meta campaign creation requests', () => {
...
it('builds Google Ads mutate operations for campaign activation draft', () => {
...
it('executes Meta create campaign requests without stub responses', async () => {
