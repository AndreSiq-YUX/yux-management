...
} from '../../src/lib/edge-compat/omnichannel.js'
...
it('normalizes inbound events for WhatsApp, Instagram, email and webchat', () => {
...
it('builds outbound adapter payloads without credentials', () => {
...
it('sanitizes protected errors and webhook metadata', () => {
...
it('calculates AI run cost from token usage', () => {
...
it('builds CRM sync payloads with sanitized metadata', () => {
...
it('builds CRM AI insight payloads from provider metadata', () => {
...
it('plans AI processing by response mode', () => {
...
it('selects published knowledge snapshots only', () => {
...
it('builds safe AI fallback metadata for n8n failures', () => {
...
it('increments outbound retry attempts without duplicating messages', () => {
...
it('preserves pending scheduling requests when n8n is not configured', () => {
