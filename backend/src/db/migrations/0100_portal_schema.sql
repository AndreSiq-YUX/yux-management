...
CREATE TABLE IF NOT EXISTS public.email_send_requests (
  ...
  sender_scope TEXT NOT NULL DEFAULT 'system' CHECK (sender_scope IN ('system', 'organization')),
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_email_templates_global_blueprint_key ON public.email_templates(scope, blueprint_key) WHERE blueprint_key IS NOT NULL AND scope IN ('system', 'blueprint');
