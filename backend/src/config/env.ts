...
const envSchema = z.object({
  ...
  YUX_WORKER_QUEUE_CLASS: optionalString,
  YUX_DRAIN_LEGACY_QUEUE: optionalBoolean,
  YUX_SCHEDULER_ENABLED: optionalBoolean,
  YUX_INTERACTIVE_CONCURRENCY: z.coerce.number().int().min(1).max(50).optional(),
  YUX_INGESTION_CONCURRENCY: z.coerce.number().int().min(1).max(50).optional(),
  YUX_EXTERNAL_CONCURRENCY: z.coerce.number().int().min(1).max(50).optional(),
  YUX_MAINTENANCE_CONCURRENCY: z.coerce.number().int().min(1).max(50).optional(),
  ...
  MISSION_DECISIONS_ENABLED: optionalBoolean,
  MISSION_DECISION_NOTIFICATIONS_ENABLED: optionalBoolean,
  MISSION_SIMULATION_REPORTS_ENABLED: optionalBoolean,
  MISSION_DECISION_FEEDBACK_ENABLED: optionalBoolean,
  MISSION_CONVERSATIONS_ENABLED: optionalBoolean,
  MISSION_CONVERSATIONS_TENANT_ALLOWLIST: optionalString,
  MISSION_CONVERSATIONS_MAX_TURNS: z.coerce.number().int().min(1).max(20).optional(),
  ...
  JINA_REQUEST_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(120_000).optional(),
  ...
  STRATEGY_INGESTION_MAX_MB: z.coerce.number().int().min(1).max(256).optional(),
