...
export async function adjustProviderCampaignBudget(pool:Connectable,context:MissionCommandContext,input:{
  ...
  const prepared=await transaction(pool,async client=>{
    ...
    const requestHash=missionArtifactHash(requestPayload);const intent=await requireActionProviderIntent(client,context,requestHash,input.approvedSubjectHash)
    const idempotencyKey=`${provider.provider}:update_budget:${intent.intentId}`
    const run=await client.query<{id:string;status:string;payload_hash:string;approval_id:string|null}>(`INSERT INTO public.ad_provider_mutation_runs (organization_id,provider_connection_id,campaign_id,provider,action,status,idempotency_key,request_payload,request_hash,payload_hash,approved_subject_hash,mission_id,action_run_id,requested_by,intent_id,approval_id,approval_source,external_effect_id) VALUES ($1,$2,$3,$4,'update_budget','running',$5,$6,$7,$7,$8,$9,$10,$11,$12,$13,'action',$14) ON CONFLICT (idempotency_key) DO UPDATE SET idempotency_key=EXCLUDED.idempotency_key RETURNING id,status,payload_hash,approval_id`,[context.organizationId,provider.id,row.id,provider.provider,idempotencyKey,sanitizeProviderMetadata(requestPayload),requestHash,input.approvedSubjectHash,context.missionId,context.actionRunId,context.actorId,intent.intentId,intent.approvalId,intent.effectId])
    if(run.rows[0]?.payload_hash!==requestHash||run.rows[0]?.approval_id!==intent.approvalId)throw new Error('provider_intent_identity_conflict')
    if(['unknown','manual_review'].includes(run.rows[0]?.status??''))throw new Error('provider_reconciliation_required')
    return{version,provider,requestPayload,runId:required(run.rows[0]?.id),campaignId:row.id,providerReference:row.external_id,intentId:intent.intentId,approvalId:intent.approvalId}
  })
  ...
  await assertActionProviderApproval(pool,context,prepared.approvalId,input.approvedSubjectHash)
  const response=await executeProviderAdapter({provider:prepared.provider.provider,action:'update_budget',localMutationId:prepared.intentId,requestPayload:{...prepared.requestPayload,accessToken:secret.value,providerAccountId:prepared.provider.provider_account_id??undefined,intentId:prepared.intentId},...(input.fetcher?{fetcher:input.fetcher}:{})})
...
async function providerMutation(pool:Connectable,context:MissionCommandContext,input:{action:AdsProviderMutationAction;commandKey:string;versionId:string;expectedContentHash:string;approvedSubjectHash:string;maxTotalBudgetBrl?:string;fetcher?:typeof fetch;nextStatus:'provider_paused'|'active'|'paused'}):Promise<ProviderResult>{
  const prepared=await transaction(pool,async client=>{
    ...
    const provider=connection.rows[0];if(!provider||provider.status!=='connected'||!provider.token_reference)throw new Error('campaign_provider_connection_unavailable')
    const campaignId=required(version.campaignId);const requestPayload=await buildProviderPayload(client,version,input.action)
    const requestHash=missionArtifactHash(requestPayload);const intent=await requireActionProviderIntent(client,context,requestHash,input.approvedSubjectHash);const idempotencyKey=`${provider.provider}:${input.action}:${intent.intentId}`
    const run=await client.query<{id:string;status:string;provider_reference:string|null;payload_hash:string;approval_id:string|null}>(`INSERT INTO public.ad_provider_mutation_runs (organization_id,provider_connection_id,campaign_id,provider,action,status,idempotency_key,request_payload,request_hash,payload_hash,approved_subject_hash,mission_id,action_run_id,requested_by,intent_id,approval_id,approval_source,external_effect_id) VALUES ($1,$2,$3,$4,$5,'pending',$6,$7,$8,$8,$9,$10,$11,$12,$13,$14,'action',$15) ON CONFLICT (idempotency_key) DO UPDATE SET idempotency_key=EXCLUDED.idempotency_key RETURNING id,status,provider_reference,payload_hash,approval_id`,[context.organizationId,provider.id,campaignId,provider.provider,input.action,idempotencyKey,sanitizeProviderMetadata(requestPayload),requestHash,input.approvedSubjectHash,context.missionId,context.actionRunId,context.actorId,intent.intentId,intent.approvalId,intent.effectId])
    if(run.rows[0]?.payload_hash!==requestHash||run.rows[0]?.approval_id!==intent.approvalId)throw new Error('provider_intent_identity_conflict')
    if(['unknown','manual_review'].includes(run.rows[0]?.status??''))throw new Error('provider_reconciliation_required')
    ...
    return{version,provider,requestPayload,runId:required(run.rows[0]?.id),intentId:intent.intentId,approvalId:intent.approvalId}
  })
  if('prior'in prepared&&prepared.prior)return prepared.prior
  if(!('provider'in prepared))throw new Error('campaign_provider_preparation_failed')
  const secret=await loadProviderSecretFromPool(pool as never,required(prepared.provider.token_reference??undefined));if(secret.expired)throw new Error('campaign_provider_token_expired')
  await assertActionProviderApproval(pool,context,prepared.approvalId,input.approvedSubjectHash)
  const response=await executeProviderAdapter({provider:prepared.provider.provider,action:input.action,localMutationId:prepared.intentId,requestPayload:{...prepared.requestPayload,accessToken:secret.value,providerAccountId:prepared.provider.provider_account_id??undefined,intentId:prepared.intentId},...(input.fetcher?{fetcher:input.fetcher}:{})})
...
async function requireActionProviderIntent(client:MissionCommandQueryable,context:MissionCommandContext,payloadHash:string,approvedSubjectHash:string){
  const result=await client.query<{effect_id:string;intent_id:string;request_hash:string;status:string;approval_id:string|null;effect_subject_hash:string|null;approval_status:string|null;approval_subject_hash:string|null}>(`SELECT effect.id AS effect_id,effect.intent_id,effect.request_hash,effect.status,effect.approval_id,effect.approval_subject_hash AS effect_subject_hash,approval.status AS approval_status,approval.subject_hash AS approval_subject_hash FROM public.action_external_effects effect LEFT JOIN public.action_approvals approval ON approval.id=effect.approval_id AND approval.organization_id=effect.organization_id WHERE effect.run_id=$1 AND effect.organization_id=$2 FOR UPDATE`,[context.actionRunId,context.organizationId]);const row=result.rows[0]
  if(!row||row.status!=='dispatched')throw new Error('provider_effect_intent_unavailable')
  if(!row.approval_id||row.approval_status!=='approved')throw new Error('provider_effect_approval_required')
  if(row.effect_subject_hash!==approvedSubjectHash||row.approval_subject_hash!==approvedSubjectHash)throw new Error('provider_effect_approval_subject_changed')
  return{effectId:row.effect_id,intentId:row.intent_id,approvalId:row.approval_id,actionPayloadHash:row.request_hash,providerPayloadHash:payloadHash}
}
async function assertActionProviderApproval(pool:Connectable,context:MissionCommandContext,approvalId:string,subjectHash:string){const result=await pool.query<{id:string}>(`SELECT id FROM public.action_approvals WHERE id=$1 AND run_id=$2 AND organization_id=$3 AND status='approved' AND subject_hash=$4 LIMIT 1`,[approvalId,context.actionRunId,context.organizationId,subjectHash]);if(!result.rows[0])throw new Error('provider_effect_approval_revoked')}
